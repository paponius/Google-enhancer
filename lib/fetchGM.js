...
async function fetchGM(resource, options = {}, responseType = 'stream') {
	...
	return new Promise((resolve, reject) => {
		...
		var abort;
		options.onabort = response => { if (LOG) { console.debug('[Fetch_GM] event: abort', options.url, response); } };
		...
		options.onprogress = response => { if (LOG) { console.log(`[Fetch_GM] event: progress: ${response.loaded}`, options.url, response); } };
		...
		const fetchPromise = new Promise((resolveFetchDone, rejectFetchDone) => {
			...
			if (LOG) { console.log('[fetchGM.js] calling GM_xmlhttpRequest url:', options.url); }
			if (LOG) { console.debug('[fetchGM.js] ... options:', options); }
		...
		function processResponse(response, event, url) {
			if (LOG) { console.debug(`[Fetch_GM] event: ${event}; url: ${url}; readyState: ${response.readyState}; status: ${response.status}; response:`, response); }
			...
			if (LOG) { console.log('[fetchGM.js] resolving XHR url:', url); }
			if (!response.response) {
				// note 2512: happens with readyState: 2, when object does not have .response, but has .responseHeaders
				//            is this an error? with GM? or what? what to do? retry? silent ignore?
				debugger;
			...
			Object.defineProperty(responseReal, 'headers', {
				...
				get() {
					if (LOG) { console.debug('[fetchGM.js] get headers', url); }
